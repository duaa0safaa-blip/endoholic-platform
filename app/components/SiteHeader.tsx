...
export default function SiteHeader() {
  ...
  return (
    <header className="w-full flex flex-wrap items-center justify-between gap-3 py-3 px-4 sm:px-6 bg-slate-900/60 backdrop-blur-md border-b border-white/10 shadow-xl z-20 sticky top-0">
      <Link href="/" aria-label="Endoholic home" className="flex items-center gap-3 shrink-0">
        ...
        <div className="hidden sm:block leading-tight">
          ...
          <p className="text-[10px] text-teal-200">Safe Endodontic Instrumentation</p>
        </div>
      </Link>

      <nav aria-label="Primary" className="flex flex-wrap items-center justify-end gap-3 sm:gap-4">
        <div className="flex items-center gap-4">
          <a href="/about" className="text-slate-200 hover:text-white text-sm">The Book</a>
          <a href="/pricing" className="text-slate-200 hover:text-white text-sm">Buy</a>
        ...
        {isSubscribed ? (
          <span className="text-xs bg-teal-950/80 border border-teal-500 text-teal-300 px-3 py-1.5 rounded-xl font-bold flex items-center gap-1.5 shadow">
            ...
            <span className="sr-only">You have verified access to the book</span>
            Book Access Verified
          </span>
        ) : (
          <button
